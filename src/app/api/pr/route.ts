...
export async function POST(request: Request) {
  ...
  try {
    ...
    // PRs start with WAITING_APPROVAL
    const status = 'WAITING_APPROVAL';

    // Generate PR number
    // Use format PR-YYYYMMDD-XXXX
    const date = new Date();
    const dateStr = date.toISOString().slice(0, 10).replace(/-/g, '');
    const randomSuffix = Math.floor(1000 + Math.random() * 9000);
    const rfcNumber = `PR-${dateStr}-${randomSuffix}`;
    ...
    return NextResponse.json({ message: 'PR created successfully', data: { id: id } }, { status: 201 });
  } catch (error: any) {
