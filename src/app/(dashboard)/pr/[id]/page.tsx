...
export default function RfcDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await api.patch(`/api/pr/${resolvedParams.id}`, { 
        status: 'APPROVED', 
  ...
  const fetchRfcDetails = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/api/pr/${resolvedParams.id}`);
      setRfc(data.data);
    } catch (error: any) {
  ...
  if (!rfc) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <h2 className="text-xl font-semibold mb-2">PR Not Found</h2>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        <div className="flex items-start md:items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Detailed view of Purchase Request</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2 md:ml-auto pl-12 md:pl-0">
          ...
          <Button variant="default" size="sm" className="sm:size-default" onClick={() => window.open(`/print/pr/${rfc.id}`, '_blank')}>
      ...
      <Dialog open={isApproveModalOpen} onOpenChange={setIsApproveModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to approve PR {rfc?.rfcNumber}. If required, please upload the signed document below.
            </DialogDescription>
          </DialogHeader>
