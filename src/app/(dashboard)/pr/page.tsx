...
export default function RfcPage() {
 ...
 const fetchRfcs = async () => {
   setLoading(true);
   try {
     const { data } = await api.get('/api/pr', { params: { search, status, startDate, endDate, sort, limit: 100 } });
     setRfcs(data.data || []);
 } catch (error) {
 console.error(error);
 } finally {
 ...
 const handleEditSubmit = async (e: React.FormEvent) => {
   ...
   try {
     await api.patch(`/api/pr/${selectedRfc.id}`, {
       isEdit: true,
       location: editFormData.location,
       notes: editFormData.notes
     });
     toast.success('PR updated and reset to DRAFT');
     setEditModalOpen(false);
     fetchRfcs();
   } catch (error: any) {
     toast.error(error.response?.data?.message || 'Failed to update PR');
   } finally {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/api/pr/${deleteId}`);
      toast.success('PR deleted successfully');
      setDeleteId(null);
      fetchRfcs();
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to delete PR');
    } finally {
      setIsDeleting(false);
    }
  };
 ...
 <div className="animate-fade-in">
        <h1 className="text-3xl font-bold tracking-tight">PR Management</h1>
 <p className="text-sm text-muted-foreground mt-1">Manage Purchase Request (PR) workflows</p>
      </div>
      ...
      <div className="flex flex-col gap-4 animate-fade-in" style={{ animationDelay: '100ms' }}>
        <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            ...
            <Link href="/pr/create" className="ml-auto lg:ml-2">
  ...
  <TableCell className="text-right">
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem className="cursor-pointer" onClick={() => window.location.href = '/pr/' + rfc.id}>
 ...
 <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
  <DialogContent>
    <form onSubmit={handleEditSubmit}>
      <DialogHeader>
        <DialogTitle>Edit PR {selectedRfc?.rfcNumber}</DialogTitle>
        <DialogDescription>
          Modifying this PR will reset its status back to Draft and it will require re-approval.
        </DialogDescription>
      </DialogHeader>
 ...
 <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
  ...
  <DialogHeader>
  <DialogTitle>Delete PR?</DialogTitle>
  <DialogDescription>
  Are you sure you want to delete this PR? This cannot be undone.
  </DialogDescription>
  </DialogHeader>
