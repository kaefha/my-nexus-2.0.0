...
export default function RfcApprovalPage() {
 ...
 try {
 const { data } = await api.get('/api/pr', { params: { search, limit: 50 } });
  // Filter for PRs that need approval based on user role and strict destination
      ...
      if (actionId) {
        ...
        if (found) {
          ...
          if (typeof window !== 'undefined') {
            window.history.replaceState({}, '', '/pr/approval');
          }
        }
      }
 } catch (error) {
 ...
 setLoading(false);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await api.patch(`/api/pr/${approvingRfc.id}`, { 
        status: nextStatus, 
  ...
  const handleReject = async () => {
    ...
    try {
      await api.patch(`/api/pr/${rejectingRfcId}`, { status: 'REJECTED', approverId: user?.id });
      fetchPendingRfcs();
      window.dispatchEvent(new Event('refreshNotifications'));
 ...
 <div className="animate-fade-in">
        <h1 className="text-3xl font-bold tracking-tight">PR Approval Queue</h1>
 <p className="text-sm text-muted-foreground mt-1">Review and approve Purchase Request workflows</p>
      </div>
 ...
 <TableRow>
 <TableHead className="w-[120px]">PR Number</TableHead>
 ...
 <p className="text-sm text-muted-foreground mt-1">There are no PRs waiting for your approval right now.</p>
 </div>
 )}
        </div>
      </div>
      ...
      <Dialog open={!!approvingRfc} onOpenChange={(open) => !open && setApprovingRfc(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Approve PR</DialogTitle>
            <DialogDescription>
              You are about to approve PR {approvingRfc?.rfcNumber}. If required, please upload the signed document below.
            </DialogDescription>
          </DialogHeader>
